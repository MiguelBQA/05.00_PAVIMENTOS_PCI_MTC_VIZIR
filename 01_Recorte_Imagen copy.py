...
class RoadImageProcessor:
    ...
    @staticmethod
    def scan_tile_groups(root_dir: str, include_subfolders: bool = True) -> List[dict]:
        ...
        for file_path in iterator:
            ...
            groups.setdefault(key, {
                ...
                'prefix': prefix,
                'tiles': []
            })['tiles'].append((index, str(file_path)))

        result = []
        for group in groups.values():
            tiles = sorted(group['tiles'], key=lambda item: (item[0], item[1]))
            ...
            rel_folder = os.path.relpath(group['folder'], root_dir)
            if rel_folder == "." or os.path.basename(group['folder']) == group['prefix']:
    ...
    def build_join_batches(group: dict, start_index: int = 0, end_index: int = -1,
        ...
        tile_items = sorted(tile_items, key=lambda item: (item[0], item[1]))
...
class RoadProcessorGUI:
    ...
    def __init__(self):
        ...
        self.join_columns = tk.IntVar(value=0)
...
5. El procesamiento cubrira TODA la imagen sin dejar restos
    ...
    def _setup_join_tab(self, parent):
        ...
        folder_frame = ttk.LabelFrame(parent, text="Carpetas", padding=15)
        folder_frame.pack(fill=tk.X, pady=(0, 10))

        input_row = ttk.Frame(folder_frame)
        input_row.pack(fill=tk.X, pady=(0, 8))
        ttk.Label(input_row, text="Carpeta de recortes:", width=20).pack(side=tk.LEFT)
        ttk.Entry(input_row, textvariable=self.join_input_dir).pack(side=tk.LEFT, fill=tk.X, expand=True, padx=8)
        ttk.Button(input_row, text="Seleccionar", command=self._select_join_input_dir).pack(side=tk.LEFT)

        output_row = ttk.Frame(folder_frame)
        output_row.pack(fill=tk.X)
        ttk.Label(output_row, text="Guardar joins en:", width=20).pack(side=tk.LEFT)
        ttk.Entry(output_row, textvariable=self.join_output_dir).pack(side=tk.LEFT, fill=tk.X, expand=True, padx=8)
        ttk.Button(output_row, text="Seleccionar", command=self._select_join_output_dir).pack(side=tk.LEFT)

        options_frame = ttk.LabelFrame(parent, text="Opciones de union", padding=12)
        options_frame.pack(fill=tk.X, pady=(0, 10))

        ttk.Checkbutton(options_frame, text="Buscar tambien en subcarpetas",
                        variable=self.join_include_subfolders).pack(side=tk.LEFT, padx=(0, 18))
        ttk.Label(options_frame, text="Columnas (0=auto):").pack(side=tk.LEFT)
        ttk.Spinbox(options_frame, from_=0, to=500, width=7,
                    textvariable=self.join_columns).pack(side=tk.LEFT, padx=(6, 18))
        ttk.Label(options_frame, text="Separacion px:").pack(side=tk.LEFT)
        ttk.Spinbox(options_frame, from_=0, to=200, width=7,
                    textvariable=self.join_spacing).pack(side=tk.LEFT, padx=(6, 18))
        ttk.Button(options_frame, text="Analizar grupos",
                   command=self._scan_join_groups).pack(side=tk.RIGHT)

        range_frame = ttk.LabelFrame(parent, text="Rango e intervalo", padding=12)
        range_frame.pack(fill=tk.X, pady=(0, 10))

        ttk.Label(range_frame, text="Desde tile:").pack(side=tk.LEFT)
        ttk.Spinbox(range_frame, from_=0, to=999999, width=9,
                    textvariable=self.join_start_index).pack(side=tk.LEFT, padx=(6, 18))
        ttk.Label(range_frame, text="Hasta tile (-1=ultimo):").pack(side=tk.LEFT)
        ttk.Spinbox(range_frame, from_=-1, to=999999, width=9,
                    textvariable=self.join_end_index).pack(side=tk.LEFT, padx=(6, 18))
        ttk.Label(range_frame, text="Cada N imagenes (0=todas):").pack(side=tk.LEFT)
        ttk.Spinbox(range_frame, from_=0, to=999999, width=9,
                    textvariable=self.join_batch_size).pack(side=tk.LEFT, padx=(6, 18))

        join_mode_tabs = ttk.Notebook(parent)
        join_mode_tabs.pack(fill=tk.BOTH, expand=True)

        auto_join_tab = ttk.Frame(join_mode_tabs, padding=8)
        free_join_tab = ttk.Frame(join_mode_tabs, padding=8)
        join_mode_tabs.add(auto_join_tab, text="Automatico exportado")
        join_mode_tabs.add(free_join_tab, text="Visor libre")

        groups_frame = ttk.LabelFrame(auto_join_tab, text="Grupos detectados", padding=10)
        groups_frame.pack(fill=tk.BOTH, expand=True, pady=(0, 10))

        columns = ('grupo', 'carpeta', 'tiles', 'tamano', 'salida')
        self.join_tree = ttk.Treeview(groups_frame, columns=columns, show='headings', height=10)
        self.join_tree.heading('grupo', text='Grupo')
        self.join_tree.heading('carpeta', text='Carpeta')
        self.join_tree.heading('tiles', text='Tiles')
        self.join_tree.heading('tamano', text='Tamano tile')
        self.join_tree.heading('salida', text='Archivo de salida')
        self.join_tree.column('grupo', width=230, anchor=tk.W)
        self.join_tree.column('carpeta', width=360, anchor=tk.W)
        self.join_tree.column('tiles', width=80, anchor=tk.CENTER)
        self.join_tree.column('tamano', width=110, anchor=tk.CENTER)
        self.join_tree.column('salida', width=230, anchor=tk.W)
        self.join_tree.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)

        join_scroll = ttk.Scrollbar(groups_frame, command=self.join_tree.yview)
        join_scroll.pack(side=tk.RIGHT, fill=tk.Y)
        self.join_tree.config(yscrollcommand=join_scroll.set)

        action_frame = ttk.Frame(auto_join_tab)
        action_frame.pack(fill=tk.X, pady=(0, 10))

        self.btn_join_selected = ttk.Button(action_frame, text="Juntar seleccionados",
                                            style='Secondary.TButton',
                                            command=self._start_join_selected,
                                            state=tk.DISABLED)
        self.btn_join_selected.pack(side=tk.RIGHT, padx=5)

        self.btn_join_all = ttk.Button(action_frame, text="Juntar todos",
                                       style='Action.TButton',
                                       command=self._start_join_all,
                                       state=tk.DISABLED)
        self.btn_join_all.pack(side=tk.RIGHT, padx=5)

        log_frame = ttk.LabelFrame(auto_join_tab, text="Registro Join", padding=10)
        log_frame.pack(fill=tk.BOTH, expand=True)

        self.join_status_text = tk.Text(log_frame, height=8, wrap=tk.WORD,
                                        font=(UI_MONO_FONT_FAMILY, 10))
        ...
        self._setup_join_free_tab(free_join_tab)

    def _setup_join_free_tab(self, parent):
        """Visor para juntar imagenes comunes en el orden elegido por el usuario."""
        parent.columnconfigure(0, weight=3)
        parent.columnconfigure(1, weight=2)
        parent.rowconfigure(0, weight=1)

        list_frame = ttk.LabelFrame(parent, text="Imagenes para juntar", padding=10)
        ...
        self.join_free_tree.column('orden', width=55, anchor=tk.CENTER)
        self.join_free_tree.column('archivo', width=220, anchor=tk.W)
        self.join_free_tree.column('tamano', width=95, anchor=tk.CENTER)
        self.join_free_tree.column('carpeta', width=320, anchor=tk.W)
        ...
        order_frame.grid(row=2, column=0, sticky="ew", pady=(8, 0))
        ...
        self.btn_join_free_selected = ttk.Button(
            order_frame,
            text="Juntar seleccionadas",
            style='Secondary.TButton',
            command=self._start_join_free_selected,
            state=tk.DISABLED
        )
        ...
        self.btn_join_free_all = ttk.Button(
            ...
            text="Juntar visor",
            style='Action.TButton',
            command=self._start_join_free_all,
            state=tk.DISABLED
        )
        self.btn_join_free_all.pack(side=tk.RIGHT, padx=5)

        preview_frame = ttk.LabelFrame(parent, text="Visor", padding=10)
        preview_frame.grid(row=0, column=1, sticky="nsew")
        preview_frame.rowconfigure(0, weight=1)
        preview_frame.columnconfigure(0, weight=1)

        self.join_free_canvas = tk.Canvas(preview_frame, bg=self.colors['canvas'])
        self.join_free_canvas.grid(row=0, column=0, sticky="nsew")
        _style_canvas_widget(self.join_free_canvas, self.colors['canvas'])
        self.join_free_canvas.bind('<Configure>', lambda event: self._render_join_free_selected_preview())

        self.join_free_info_label = ttk.Label(preview_frame, text="Sin imagen seleccionada", style='Info.TLabel')
        self.join_free_info_label.grid(row=1, column=0, sticky="ew", pady=(8, 0))
    ...
    def _refresh_join_free_tree(self, selected_indices=None):
        ...
        if selected_indices:
            ...
            if valid:
                ...
                self._render_join_free_preview(self.join_free_images[int(valid[0])])
                return

        if self.join_free_images:
            self.join_free_tree.selection_set("0")
            self.join_free_tree.focus("0")
            self._render_join_free_preview(self.join_free_images[0])
        else:
            self._clear_join_free_preview()
    ...
    def _on_join_free_select(self, event=None):
        self._render_join_free_selected_preview()

    def _render_join_free_selected_preview(self):
        if not hasattr(self, 'join_free_tree') or not self.join_free_images:
            self._clear_join_free_preview()
            return

        selected = self.join_free_tree.selection()
        if not selected:
            self._clear_join_free_preview()
            return
        ...
        try:
            idx = int(selected[0])
            self._render_join_free_preview(self.join_free_images[idx])
    ...
    def _render_join_free_preview(self, record):
        try:
            with Image.open(record['path']) as img:
                ...
                max_w = max(80, canvas_w - 24)
                max_h = max(80, canvas_h - 24)
                img.thumbnail((max_w, max_h), Image.LANCZOS)

                self.join_free_preview_photo = ImageTk.PhotoImage(img)
                self.join_free_canvas.delete("all")
                x_off = (canvas_w - img.width) // 2
                y_off = (canvas_h - img.height) // 2
                self.join_free_canvas.create_image(x_off, y_off, anchor=tk.NW, image=self.join_free_preview_photo)
                self.join_free_info_label.config(text=f"{record['name']} | {record['size']}")
        except Exception as e:
            self._clear_join_free_preview(f"No se pudo mostrar: {e}")

    def _clear_join_free_preview(self, message="Sin imagen seleccionada"):
    ...
    def _scan_join_groups(self):
        ...
        if groups:
            ...
            self._log_join(f"Grupos detectados: {len(groups)} | Tiles: {total_tiles}")
            self._get_join_output_dir()
        else:
            self._log_join("No se encontraron tiles con patron nombre_000000.png/tif")
            messagebox.showinfo("Sin grupos", "No se encontraron recortes generados por este programa")
